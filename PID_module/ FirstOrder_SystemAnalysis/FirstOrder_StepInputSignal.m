...
% Input signal (step)
u_step = ones(size(t)); % Step input signal
% Simulating the response of the system to the step input
[y_step, t_step] = lsim(G, u_step, t);
% Plotting the response of the system to the step input
subplot(2, 1, 1); % Creating subplot for the response
plot(t_step, y_step);
xlabel('Time'); % Label for x-axis
ylabel('Output'); % Label for y-axis
title('System Response to Step Input'); % Title for the plot
